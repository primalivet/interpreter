...
let step ?(n = 1) l =
  ...
  if next_pos >= String.length l.input
  then { l with pos = next_pos; ch = None }
  else { l with pos = next_pos; ch = Some (String.get l.input next_pos) }
;;

let peek l =
  let next_pos = l.pos + 1 in
  if next_pos >= String.length l.input then None else Some (String.get l.input next_pos)
;;
...
let skip ~pred l =
  let rec inner l =
    match l.ch with
    | None -> l
    | Some c -> if pred c then inner (step l) else l
  in
  inner l
;;
...
let eat l =
  ...
  | Some c when Char.is_digit c ->
    take_all ~pred:Char.is_digit l |> fun (s, l) -> Token.Int (Int.of_string s), l
  | Some _ -> Token.Illegal, step l
;;

let init s =
  if String.length s > 0
  then { input = s; pos = 0; ch = Some (String.get s 0) }
