...
let tests =
  "tests"
  >::: [
         make_lexer_result_test "eat one token"
           (Token.Plus, { input = "+"; pos = 1; ch = None })
           ("+" |> Lexer.init |> Lexer.eat);
         make_lexer_result_test "eat two tokens"
           (Token.Eof, { input = "+"; pos = 2; ch = None })
           ("+" |> Lexer.init |> Lexer.eat |> snd |> Lexer.eat);
         make_lexer_result_test "eat tree tokens"
           (Token.Eof, { input = "+"; pos = 3; ch = None })
           ("+" |> Lexer.init |> Lexer.eat |> snd |> Lexer.eat |> snd
          |> Lexer.eat);
         make_lexer_collect_test "two-symbol operators for (==)"
           [ Token.Assign; Token.Eq; Token.Eq; Token.Assign; Token.Eof ]
           "= == ===";
         make_lexer_collect_test "two-symbol operators for (!=)"
           [ Token.Bang; Token.NotEq; Token.NotEq; Token.Assign; Token.Eof ]
           "! != !==";
         make_lexer_collect_test "parses book example program (ch1)"
           [
             Token.Let;
             Token.Ident "five";
             Token.Assign;
